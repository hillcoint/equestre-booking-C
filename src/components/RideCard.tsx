...
interface RideCardProps { ride: Ride; freePlaces: number; dateBlocked?: boolean; onBook: (ride: Ride) => void; }

export function RideCard({ ride, freePlaces, dateBlocked = false, onBook }: RideCardProps) {
  const blocked = ride.status === "blocked" || dateBlocked;
  return <article className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4"><div><p className="text-sm font-semibold text-emerald-700">{ride.level}</p><h3 className="mt-1 text-xl font-bold">{ride.title}</h3></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${blocked ? "bg-red-100 text-red-700" : freePlaces > 0 ? "bg-emerald-100 text-emerald-800" : "bg-stone-200 text-stone-700"}`}>{dateBlocked ? "Fecha bloqueada" : ride.status === "blocked" ? "Cancelada" : freePlaces > 0 ? `${freePlaces} libres` : "Completa"}</span></div>
