...
import { CalendarPlus, LockKeyhole, MessageCircle, Users, X } from "lucide-react";
...
export function AdminPanel({ rides, bookings, onClose, onAddRide, onToggleRide }: AdminPanelProps) {
  ...
  const shareWhatsApp = () => {
    ...
    const list = rideBookings.length ? rideBookings.map((booking, index) => `${index + 1}. ${booking.name} — ${booking.people} persona(s) — ${booking.phone}`).join("\n") : "Sin reservas todavía.";
    const message = `Senda Ecuestre\n${selectedRide.title}\n${selectedRide.date} · ${selectedRide.time}\n\nReservas (${participants} participantes):\n${list}`;
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-950/60 p-4 md:p-8"><div className="mx-auto max-w-6xl rounded-3xl bg-stone-50 shadow-2xl">
    ...
    <div className="grid gap-6 p-6 lg:grid-cols-[.85fr_1.15fr]">
      ...
      <section className="rounded-2xl border bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-bold text-emerald-700">SALIDAS Y RESERVAS</p><h3 className="text-lg font-bold">Gestión del día</h3></div>{selectedRide && <button onClick={shareWhatsApp} className="flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2 text-sm font-bold text-white"><MessageCircle size={18}/> Compartir por WhatsApp</button>}</div>
        <select className="mt-5 w-full rounded-lg border p-3" value={selectedRideId} onChange={e=>setSelectedRideId(e.target.value)}>{rides.map(ride=><option key={ride.id} value={ride.id}>{ride.date} · {ride.time} · {ride.title}</option>)}</select>
        {selectedRide && <><div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-stone-100 p-4"><div><p className="font-bold">{selectedRide.title}</p><p className="text-sm text-stone-600"><Users className="mr-1 inline" size={16}/>{participants} / {selectedRide.capacity} participantes reservados</p></div><button onClick={()=>onToggleRide(selectedRide.id)} className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-bold ${selectedRide.status === "blocked" ? "bg-emerald-700 text-white" : "bg-red-100 text-red-700"}`}><LockKeyhole size={17}/>{selectedRide.status === "blocked" ? "Reabrir ruta" : "Bloquear por seguridad"}</button></div>
